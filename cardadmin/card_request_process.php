...
include 'connection.php';
    ...
    $query="SELECT *, card.id AS cardid  FROM card INNER JOIN card_type ON card_type.id=card.card_typeid WHERE gmail='$usergmail'";
    ...
    if(isset($_POST['accept'])){
      ...
      if($cat=="first class"){
        $editdiscount=20;  
      }elseif ($cat=="business class") {
      	$editdiscount=16;
      }elseif ($cat=="premium economy class") {
      	$editdiscount=12;
      }elseif ($cat=="economy class") {
      	$editdiscount=8;
      }
...
if($result)
    {
      header("location: card_request_table.php?message=request accepted!");
}
else{
    header("location: card_request_table.php?message=request canceled!");
}
    ...
    $id=$_POST['btnid'];
      $edit="reject";
           ...
           $query="UPDATE `card` SET status = '$edit' WHERE id = '$id'";
